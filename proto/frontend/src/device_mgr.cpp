  ...
  namespace local {
    // WaitMyTurn causes the sender to wait until it is its turn: e.g., two senders trying to send.
	  static uint64_t WaitMyTurn(pi::fe::local::TableHeaders *h, uint64_t numUpdates) {
		  if (pthread_mutex_lock(&h->writerMutex)) {
			  errExit("pthread_mutex_lock");    
		  }

		  if (pthread_mutex_lock(&h->devMutex)) {
			  errExit("pthread_mutex_lock");    
		  }
		  // TODO: if h->done, we will hang forever.
		  assert(h->lastSendCnt==INVALID_SEND_CNT);
		  assert(h->lastSent==h->lastRecvd);
		  assert(h->head == h->tail);
		  h->lastSendCnt = numUpdates;
		  uint64_t lastRecvd = h->lastRecvd.load(memory_order_acquire);
		  h->lastSent = (lastRecvd+1);
		  pthread_cond_signal(&h->condVar);
		  pthread_mutex_unlock(&h->devMutex);
		  return lastRecvd+1;
	  }
	  ...
	  static  uint64_t WaitForData(pi::fe::local::TableHeaders *h, uint64_t &newID) {
		  ...
		  while (1) {
			  // Prioritize dev shutdown over data.
			  if (h->done) {
				  pthread_mutex_unlock(&h->devMutex);
				  pthread_exit(0);
			  }
			  uint64_t itemCount = h->lastSendCnt;
			  if (itemCount == INVALID_SEND_CNT) {
				  pthread_cond_wait(&h->condVar, &h->devMutex);
				  continue;
			  }
			  newID=h->lastSent;
			  pthread_mutex_unlock(&h->devMutex);
	  ...
	  static void SendAck(pi::fe::local::TableHeaders *h, uint64_t newId) {
		  // Reset lastSendCnt.
		  h->lastSendCnt = INVALID_SEND_CNT;
		  // Reset head and tail.
	  ...
	  static void InitSHMMutex(pthread_mutex_t *mutex) {
		  pthread_mutexattr_t writerMutexAttr;
		  if (pthread_mutexattr_init(&writerMutexAttr)) {
			  errExit("pthread_mutexattr_init");
		  }
		  if (pthread_mutexattr_setpshared(&writerMutexAttr, PTHREAD_PROCESS_SHARED)) {
			  errExit("pthread_mutexattr_setpshared");
		  }
		  if (pthread_mutex_init(mutex, &writerMutexAttr)) {
			  errExit("pthread_mutex_init");
		  }
		  if (pthread_mutexattr_destroy(&writerMutexAttr)) {
			  errExit("pthread_mutexattr_destroy");
		  }
	  }


	  static void InitSHMCondVar(pthread_cond_t *condVar) {
		  ...
		  if (pthread_cond_init(condVar, &condAttr)) {
			  errExit("pthread_cond_init");
		  }
		  if (pthread_condattr_destroy(&condAttr)) {
			  errExit("pthread_condattr_destroy");
		  }
	  }
...
#pragma omp barrier
		  ...
		  pthread_mutex_unlock(&h->writerMutex);
...
static void writeLocal(pi::fe::local::TableHeaders *h,
	...
	while(1) {
		...
		// Until there is data.
		uint64_t newID;
		uint64_t count = pi::fe::local::WaitForData(h, newID); // May pthread_exit()
 ...
 public:
  explicit DeviceMgrImp(device_id_t device_id)
	...
	device_id(device_id),
		...
		pi::fe::local::InitSHMMutex(&h->writerMutex);
		pi::fe::local::InitSHMMutex(&h->devMutex);
		pi::fe::local::InitSHMCondVar(&h->condVar);
		h->done = false;

		// Create writer thread
		writerThread = std::thread(writeLocal, h, data, reader_buffer);
#endif // HAVE_SHM
	}
  ...
  ~DeviceMgrImp() {
	  ...
	  pthread_mutex_destroy(&h->writerMutex);
	  pthread_mutex_destroy(&h->devMutex);
	  pthread_cond_destroy(&h->condVar);
